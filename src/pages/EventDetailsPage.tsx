...
const EventDetailsPage: React.FC = () => {
    ...
    const fetchReviews = async (page = 1) => {
        ...
        try {
            ...
            setReviewMeta({ page: result.meta.page, totalPages: result.meta.totalPages, total: result.meta.total });
            if (result.stats) {
            ...
            <div className="card lg:card-side bg-base-100 shadow-xl mb-6">
                ...
                <div className="card-body lg:w-1/2">
                    ...
                    <div className="card-actions justify-end">
                        <Link to={`/checkout/${id}`} className="btn btn-primary btn-lg">Book Tickets</Link>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-base-100 p-6 rounded-lg shadow-xl mb-6">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-6 mb-6 p-4 bg-base-200 rounded-lg">
                    <div className="text-center">
                        <div className="text-5xl font-bold text-primary">{reviewStats.average_rating.toFixed(1)}</div>
                        <StarRating rating={reviewStats.average_rating} size="text-xl" />
                        <div className="text-sm text-gray-500 mt-1">{reviewStats.total_reviews} review{reviewStats.total_reviews !== 1 ? 's' : ''}</div>
                    </div>
                </div>
                ...
                {eligibilityChecked && canReview && (
                    <div className="mb-6 p-4 border border-base-300 rounded-lg">
                        ...
                        {reviewSuccess && <div className="alert alert-success mb-3 text-sm">{reviewSuccess}</div>}
                        {reviewError && <div className="alert alert-error mb-3 text-sm">{reviewError}</div>}
                        <form onSubmit={formik.handleSubmit}>
                            ...
                            <div className="form-control mb-3">
                                <label className="label"><span className="label-text font-semibold">Your Comment</span></label>
                                <textarea
                                    name="comment"
                ...
                {reviewsLoading ? (
                    ...
                    <p className="text-gray-500 text-center py-4">No reviews yet. Be the first to review this event!</p>
                ) : (
                            ...
                            <div key={review.id} className="flex gap-4 p-4 bg-base-200 rounded-lg">
                                <div className="flex-shrink-0">
                                    {review.user?.profile_picture ? (
                                        <img src={review.user.profile_picture} alt={review.user.full_name} className="w-10 h-10 rounded-full object-cover" />
                                    ) : (
                ...
                {reviewMeta.totalPages > 1 && (
                    <div className="flex justify-center mt-4">
                        <div className="join">
                            {Array.from({ length: reviewMeta.totalPages }, (_, i) => i + 1).map((page) => (
                                <button
                                    key={page}
