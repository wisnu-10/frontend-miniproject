...
const CheckoutPage: React.FC = () => {
  ...
  const handlePromotionSelect = (promoId: string) => {
    ...
    const promo = event?.promotions?.find(
      (p: Promotion) => p.id.toString() === promoId,
    );
  ...
  // Calculations
  const subtotal = Object.entries(ticketQuantities).reduce(
    (sum, [tId, qty]) => {
  ...
  const finalTotal = isDiscountSelected
    ? priceAfterDiscount
    : subtotal - (usePoints ? Math.min(points, subtotal) : 0);
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="card bg-base-100 shadow-md">
            <div className="card-body p-5">
              ...
              <div
                role="alert"
                className="alert alert-info text-xs py-2 px-3 shadow-sm rounded-md mb-2"
              >
                <svg
              ...
              <div className="form-control mt-3 bg-base-200/50 rounded-lg p-3 border border-base-200">
                <label className="label cursor-pointer justify-start gap-3 p-0">
                  <input
                    ...
                    disabled={
                      !!selectedPromotion || !!selectedCoupon || points === 0
                    }
                  />
                  <span className="label-text text-sm font-medium">
                    Use Points (Balance: {formatCurrency(points)})
                  </span>
                </label>
              </div>
            </div>
